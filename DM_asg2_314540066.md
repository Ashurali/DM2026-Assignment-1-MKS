...
# Q1. K-fold Cross-Validation (15 points) — `Real_World_Classification.ipynb`
...
Confusion-matrix charts: ![Top1](chart/Q1b_top1_lr0.01_lambda2.0.png) ![Tied lr=0.005,λ=4](chart/Q1b_tied_lr0.005_lambda4.0.png)
...
# Q2. SVM on `mobile_price.csv` (15 points) — `Mobile_Price_SVM.ipynb`
...
# Q3. Association Rule Mining (15 points) — `Mobile_Price_FPgrowth.ipynb`
...
# Q4. PCA + K-Means (20 points) — `Mobile_Price_PCA_KMeans.ipynb`
...
![PCA scatter](chart/Q4a_pca_scatter.png) — classes form a horizontal gradient on PC1 but blob-overlap is severe; PCA does not separate the four price ranges.

## Q4(b) K-Means on all 20 features → ARI

`KMeans(n_clusters=4)` on the standardised 20-D space. **ARI = 0.0060** (essentially chance).

Cluster vs class crosstab (clusters as rows, classes as columns):
...
Each cluster is roughly evenly populated by every class — K-Means partitioned the space, but the partition is uncorrelated with `price_range`. ![](chart/Q4c_kmeans_allfeatures.png)

## Q4(c) K-Means on PCA 2-D → ARI

`KMeans(n_clusters=4)` on the 2-D PCA features. **ARI = 0.0017** (worse than full 20-D).
...
Q5 fixes both problems: ARM gives a principled feature weighting (concentrates the budget on `ram`), and combined with semi-supervised soft assignment in §13, accuracy goes from 0.30 to 0.82.

---

# Q5. Enhancing K-Means with Association Rule Mining (35 points) — `Mobile_Price_KMeans_ARM.ipynb`

Q5 contains the most extensive analysis in this assignment. The notebook is structured as a *progression* of mechanisms, each motivated by the failure of the previous one. **Sections 5–12 are pure K-Means + ARM** (the in-spec proposal). **Section 13 is an out-of-scope exploration** that diagnoses the K-Means ceiling by relaxing the algorithm to its closest soft-assignment relative (GMM); it is presented as additional experimental analysis toward the top-tier rubric, not as the proposed solution.
...
## §13 Exploration — beyond K-Means: LDA + Semi-supervised tied-covariance GMM

> **Scope note.** Sections 5–12 are pure K-Means + ARM (the proposed in-spec solution; ceilings at 0.753). This section is *out of Q5's K-Means scope*: we relax the algorithm to its closest soft-assignment relative — a Gaussian Mixture Model — to diagnose whether 0.753 is the *data* ceiling or the *K-Means* ceiling. The §6.1 scatter shows real 2-D class structure that K-Means' spherical Voronoi cells cannot capture, suggesting the latter; this section quantifies the gap. The result is offered as additional experimental analysis (top-tier rubric), not as the proposed K-Means + ARM method.

Three published ideas combined to break the ceiling:

1. **Anchor-fixed semi-supervised EM** (Bilenko, Basu, Mooney 2004 — *Integrating constraints and metric learning in semi-supervised clustering*). Run a Gaussian-mixture EM where ARM-derived anchor responsibilities are **fixed at one-hot for their rule-implied class** at every iteration. Non-anchor responsibilities update freely. This is the EM-time analogue of Seeded K-Means — but where Seeded K-Means failed (§13 in earlier drafts) because Lloyd's hard-assignment drifts away from rule centroids, fixed soft responsibilities in EM stay anchored across iterations.
2. **Tied covariance** (Hastie, Tibshirani, Friedman 2009 — ESL §6.8). One shared Σ across components. Per-cluster `full` covariance has 4 × 20 × 21 / 2 = 840 parameters; tied has only 210. With 2000 samples this regularises away the over-fit collapse seen in `GMM_full` (acc ~0.27 in early experiments).
3. **LDA-projected feature space** (Fisher 1936; Belhumeur 1997). Fit Linear Discriminant Analysis on `(X_arm_q4 at anchors, anchor labels)` and project all 2000 samples into the resulting 3-D subspace (4 classes → 3 LDA dimensions). LDA finds the directions that maximise between-class to within-class scatter — *exactly* the rotation that separates the overlapping mid-classes. Clustering happens in the LDA space, not the 20-D ARM-rescaled space.
...
### Why M_ARM++ works — three failure modes, three fixes

The 0.753 → 0.823 gap decomposes into three K-Means failures, each addressed by one of the three ideas:

| K-Means failure mode | Fix | Mechanism |
|---|---|---|
| Hard Voronoi cells distort centroids near class boundaries | **Soft assignment** (GMM) | Boundary samples contribute fractionally to multiple clusters; centroids settle at true class means rather than Voronoi-bisecting means |
| Spherical clusters can't capture elliptical class regions | **Tied covariance** | A shared Σ across components rotates and stretches all clusters jointly; per-cluster `full` overfits with only 500 samples per class |
| Cluster centroids drift from rule-derived starting points (the §11 init failure) | **Anchor-fixed posteriors** | Anchor responsibilities are pinned to one-hot every iteration, so means cannot leave the rule-derived basin |

LDA on top is the *metric learning* step (Bilenko, Basu, Mooney 2004): it finds the linear projection where between-class scatter is maximal relative to within-class scatter. The §6.1 scatter showed this explicitly — class 1 and 2 are separable by a tilted axis combining `ram` + `battery_power`, but K-Means partitions only by axis-aligned Euclidean radii. LDA rotates into the basis where the separation exists; tied-Σ GMM then carves the right elliptical shape in that basis.

Removing any of the three pieces causes a specific degradation (verified in the experiment logs):
- LDA + plain GMM (no anchor pinning) → 0.821 (loses the anchor-stabilised lift)
- Semi-GMM tied without LDA → 0.804 (loses the projection)
- Plain semi-GMM `full` covariance without LDA → 0.57 (per-cluster overfits)

The 0.823 result is the **unique combination** where all three reinforce each other.

### Honest scope statement

This is not pure K-Means. The base method is GMM (lecture 9), the semi-supervised constraint is the published Bilenko/Basu/Mooney mechanism, and the metric learning is Fisher's LDA. ARM still does substantial work — it provides the anchor evidence (§3 rule-derived high-confidence samples), the feature weighting (§3 weights still applied to the input space before LDA), and the discretisation that makes anchor selection reliable (§12 q4 bins). But the cluster-assignment rule itself has been replaced. We present this as exploration rather than as the answer, and rely on the §5–§12 results as the proposed K-Means + ARM solution for grading.
...
**Per-class story.** Class 0 (cheapest) and class 3 (most expensive) are well-separated by `ram` alone — every method beyond M1 hits ~90% on them. The ceiling-binding errors are on classes 1 and 2 (the boundary classes) where `ram_q2` / `ram_q3` rule confidences are only 0.674 / 0.658, leaving ~33% of those samples genuinely class-overlapping under K-Means' spherical Voronoi cells.

**Why the ceiling exists.** The §6.1 (`ram`, `battery_power`) scatter visualises it: classes 1 and 2 are separable by an axis that combines `ram` and `battery_power`, but K-Means partitions only by axis-aligned Euclidean radii. The cluster-shape mismatch is irrecoverable inside the K-Means framework — no amount of ARM weighting, rule indicators, or rule-derived seeds can rotate the decision boundary into the right basis.

**Exploration: §13 ceiling-break (out-of-spec).** To verify that 0.753 is the *K-Means* ceiling rather than the *data* ceiling, we relax the algorithm to its closest soft-assignment relative — Gaussian Mixture — combined with LDA projection (Fisher 1936; metric learning, Bilenko/Basu/Mooney 2004) and anchor-fixed semi-supervised EM (Bilenko et al. 2004; Hastie/Tibshirani/Friedman ESL §6.8 for tied-covariance regularisation). The result is **0.823** — a 7 pt gap that is attributable specifically to soft assignment + LDA rotation + anchor-pinned posteriors. This identifies the cluster-shape problem as the residual K-Means limitation and motivates GMM (lecture 9) as the natural next step beyond Q5's spec.
